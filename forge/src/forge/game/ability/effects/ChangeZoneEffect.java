        ...
        for (final Card tgtC : tgtCards) {
            ...
            if (destination.equals(ZoneType.Library)) {
                ...
                if (destination.equals(ZoneType.Battlefield)) {
                	...
                	boolean isInStack = tgtC.isInZone(ZoneType.Stack);
                    movedCard = game.getAction().moveTo(destination, tgtC);
                    // If a card is Exiled from the stack, remove its spells from the stack
                    if (isInStack && sa.hasParam("Fizzle")) {
        ...
        if(!sa.hasParam("DifferentNames") && !sa.hasParam("ShareLandType") && !sa.hasParam("AtRandom") && totalcmc == null && !defined) {
            ...
            for (int i = 0; i < changeNum && destination != null; i++) {
                ...
                if (sa.hasParam("AtRandom")) {
                    ...
                    c = decider.getController().chooseSingleCardForZoneChange(destination, origin, sa, fetchList, i == 0 ? delayedReveal : null, "[" + (i+1) + "] " + title, !sa.hasParam("Mandatory"), decider);
                }
